...
class Solid(object):
    ...
    def centralize(self):
        ...
        self.centerTranslation = \
            ((old_div(-xspan, 2.)) - xmin, (old_div(-yspan, 2.)) - ymin, (old_div(-zspan, 2.)) - zmin)
    ...
    def _compress_write(self, filename):
        vstr = '[' + ','.join(['[%.2f,%.2f,%.2f]' % \
                               (x[0], x[1], x[2]) for x in self.vertices]) + ']'
        fstr = '[' + ','.join(['[%d,%d,%d]' % \
                               (x[0], x[1], x[2]) for x in self.faces]) + ']'
    ...
    class _Verboseness(object):
        ''' Helps suppress verbouse outputs '''
